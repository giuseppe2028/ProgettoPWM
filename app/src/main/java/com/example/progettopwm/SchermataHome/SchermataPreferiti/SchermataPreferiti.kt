...
class SchermataPreferiti : Fragment() {
    ...
    private fun setSchermata() {
        ...
        val query = "select distinct V.id,luogo,nome_struttura,recensione,prezzo,path_immagine,num_persone from Viaggio V,Preferiti P,Immagini, Persona Pe,I where P.ref_viaggio = V.id and I.ref_viaggio = V.id and Pe.id = $idP and Pe.id= P.ref_persona"
       ...
       val adapter = CustomAdapterPreferiti(lista)
        ...
        adapter.setOnClickListener(object:
            ...
            override fun Onclick(position: Int, item: ItemViewModel) {
                val query = "delete from Preferiti where ref_viaggio = ${item.id} and ref_persona = $idP"
